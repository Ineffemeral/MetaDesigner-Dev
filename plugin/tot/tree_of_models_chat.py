...
TREE_OF_MODEL_PROMPT_SUBJECT_SYSTEM = """ You are an professional information analyst."""
TREE_OF_MODEL_PROMPT_SUBJECT_USER = """ You are an information analyst who can analyze and abstract a set of words to abstract some representation categories.
...
TREE_OF_MODEL_PROMPT_STYLE_SYSTEM = """ You are an professional information analyst."""
TREE_OF_MODEL_PROMPT_STYLE_USER = """ You are an information analyst who can analyze and summarize a set of words to abstract some representation categories.
...
TREE_OF_MODEL_PROMPT_SYSTEM = """ You are an professional information analyst."""
TREE_OF_MODEL_PROMPT_USER = """ You are an information analyst who can create a Knowledge Tree according to the input categories.
...
TREE_OF_MODEL_PROMPT_ADD_MODELS_SYSTEM = """ You are an professional information analyst."""
TREE_OF_MODEL_PROMPT_ADD_MODELS_USER = """ You are an information analyst who can add some input models to an input knowledge tree according to the similarity of the model tags and the categories of the knowledge tree.

You need to place each input model into the appropriate subcategory on the tree, one by one.
Each input model must be the leaf node of the knowledge tree, and without the square brackets "[]".
...
TOT_PROMPTS_SYSTEM = """ You are an professional information analyst."""
TOT_PROMPTS_USER = """Identify and behave as five different experts that are appropriate to select one element from the input list that best matches the input prompt. 
...
class TreeOfModelsChat(object):
    ...
    def __init__(self, 
        ...
        super(TreeOfModelsChat, self).__init__()
    ...
    def llm(self, 
            input_messages, 
            model='gpt-4', # chat model, 'gpt-3.5-turbo' performs poorly
            temperature=0,
            return_usage=False,
            max_tokens=2000): 
    ...
    def build_tree_model(self):
        if not osp.exists(self.model_tree_path):
            ...
            for key in self.tree_model_infos.keys():
                print(f"--[{key}]")
                for k in self.tree_model_infos[key].keys():
                    print(f"   --[{k}]")
    ...
    def build_tree(self):
        ...
        msg1 = [
            {"role": "system", "content": TREE_OF_MODEL_PROMPT_SUBJECT_SYSTEM},
            {"role": "user", "content": TREE_OF_MODEL_PROMPT_SUBJECT_USER.format(input=tags_only)},
        ]
        response1, u1 = self.llm(msg1, temperature=0, return_usage=True)
        print("Subject: ", response1)


        msg2 = [
            {"role": "system", "content": TREE_OF_MODEL_PROMPT_STYLE_SYSTEM},
            {"role": "user", "content": TREE_OF_MODEL_PROMPT_STYLE_USER.format(input=tags_only)},
        ]
        response2, u2 = self.llm(msg2, temperature=0, return_usage=True)
        print("Style: ", response2)

        msg_tree = [
            {"role": "system", "content": TREE_OF_MODEL_PROMPT_SYSTEM},
            {"role": "user", "content": TREE_OF_MODEL_PROMPT_USER.format(style=response2, subject=response1)},
        ]
        response, u_t = self.llm(msg_tree, temperature=0, return_usage=True)
        print(response)
        ...
        msg_w_models = [
            {"role": "system", "content": TREE_OF_MODEL_PROMPT_ADD_MODELS_SYSTEM},
            {"role": "user", "content": TREE_OF_MODEL_PROMPT_ADD_MODELS_USER.format(model_tags=model_tags, tree=tree, models=model_names)},
        ]        
        tree_w_models, u_twm = self.llm(msg_w_models, temperature=0, return_usage=True)
        print(tree_w_models)
    ...
    def search_one_matched(self, inputs, search_list):
        ...
        msg_tot = [
            {"role": "system", "content": TOT_PROMPTS_SYSTEM},
            {"role": "user", "content": TOT_PROMPTS_USER.format(search_list=search_list, input=inputs)},
        ]    

        model_name, u_tot = self.llm(msg_tot, temperature=0, return_usage=True)
        print(model_name)
