...
def replace_args(paresed_cmd, prog):
    for k, v in paresed_cmd['args'].items():
        if v in prog.state.keys():
            paresed_cmd['args'][k] = prog.state[v]
        else:
            paresed_cmd['args'][k] = eval(v)
...
class WordArtSemanticInterpreter():
    step_name = 'WORDARTSEMANTIC'
    ...
    def parse(self,prog_step):
        parse_result = parse_step(prog_step.prog_str)
        replace_args(parse_result, prog_step)

        step_name = parse_result['step_name']
        char = parse_result['args']['char']
        font = parse_result['args']['font']
        prompt = parse_result['args']['prompt']
        steps = parse_result['args']['steps']
        output_var = parse_result['output_var']


        assert(step_name==self.step_name)
        return char, font, prompt, steps, output_var
    
    def execute(self,prog_step,inspect=False):
        torch.cuda.empty_cache()
        char, font, prompt, steps, output_var = self.parse(prog_step)
        if self.pipe is None:
            self.pipe = pipeline(
                'wordart-generation', 
                model="jyhe21/wordart_designer", 
                model_revision='v1.0'
            )
            # hack the model
            input = {    
                ...
                "concept": prompt,
                "iteration": steps,
                "fontpath": os.path.join(self.pipe.model_dir, "data/fonts/kaiti.ttf"),
            }
            glyphs = self.pipe.forward(input)[OutputKeys.OUTPUT_IMGS]
            
            # release pipeline to save memory
            if SAVE_MEM_MODEL  == True:
                self.pipe = None

        prog_step.state[output_var] = glyphs[0]
        return glyphs[0]

class WordArtTextureInterpreter():
    step_name = 'WORDARTTEXTURE'
    ...
    def parse(self,prog_step):
        ...
        step_name = parse_result['step_name']
        model = parse_result['args']['model']
        cond = parse_result['args']['cond']
        prompt = parse_result['args']['prompt']
        output_var = parse_result['output_var']


        assert(step_name==self.step_name)
        return model, cond, prompt, output_var
    
    def execute(self, prog_step, inspect=False):
        torch.cuda.empty_cache()
        model, cond, prompt, output_var = self.parse(prog_step)
        from tempfile import TemporaryDirectory
       
        # hack 输入文件适配
        with TemporaryDirectory() as temp_dir:
            tmp_file = os.path.join(temp_dir, 'cond.jpg')
            cond.save(tmp_file)
            input_params = {
                "image": {
                    "image_path_url": tmp_file
                },
                "prompt": prompt,
                "neg_prompt": "",
            ...
            if self.pipe is None:
                ...
                output = self.pipe(input_params)


                im = output[OutputKeys.OUTPUT_IMGS][0]
                prog_step.state[output_var] = im
...
class PromptExtensionInterpreter():
    step_name = 'PROMPTEXTENSION'
    ...
    def __init__(self):
        ...
        self.prompt_template="""Please give the detail 
        description of the following concept : {concept}
        """
    ...
    def parse(self, prog_step):
        ...
        assert(step_name==self.step_name)
        return prompt, output_var
    
    def execute(self, prog_step, inspect=False):
        prompt, output_var = self.parse(prog_step)
        self.msg = [
            {"role": "system", "content": "you are an AI robot which is good at describing the detai for the given object/concept, the answer must be less than 10 words"},
            {"role": "user", "content": self.prompt_template.format(concept=prompt)}
        ]
        ...
        print(f"prompt extension content: {response.choices[0].message.content}")
        prog = response.choices[0].message.content.lstrip('\n').rstrip('\n')
        prog_step.state[output_var] = prog

        return prog

class ModelSelectionInterpreter():
    ...
    def __init__(self):
        ...
        self.model_description_prompt_templte = 'model {model_name} can generate {style_desc} image '
    ...
    def parse(self, prog_step):
        ...
        prompt = parse_result['args']['prompt']
        # import pdb; pdb.set_trace()
        # model_infos = eval(parse_result['args']['model_infos'])
        model_infos=None
        output_var = parse_result['output_var']
        
        assert(step_name==self.step_name)
        return prompt, model_infos, output_var
    
    def execute(self, prog_step, inspect=False):
        prompt, model_infos, output_var = self.parse(prog_step)
        self.msg = [
            {"role": "system", "content": "you are an AI robot which is good at choice the right model from model dict: {'3d':'generating 3D images', 'Catoon': 'catoon image generation'}"},
            {"role": "user", "content": f"the requirement: {prompt}"}
        ]
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=self.msg,
...
def register_step_interpreters(dataset='nlvr'):
    ...
    elif dataset=='wordart':
        return dict(
            WORDARTSEMANTIC=WordArtSemanticInterpreter(),
            PROMPTEXTENSION=PromptExtensionInterpreter(),
            WORDARTTEXTURE=WordArtTextureInterpreter(),
            MODELSELECTION=ModelSelectionInterpreter(),
            RESULT=ResultInterpreter()
        )